    ...
    class MatrixFactorizationTests : TestBase
    {
      public:
        ...
        TestOutcome matrixILU0()
        {
          ...
          solver.setup(A);
          status *= verifyAnswer(*(solver.getLFactor()),
                                 rowsL_,
                                 colsL_,
                                 valsL_,
                                 "cpu");
          status *= verifyAnswer(*(solver.getUFactor()),
                                 rowsU_,
                                 colsU_,
                                 valsU_,
                                 "cpu");

          // Test ILU0 factorization when matrix values change but sparsity is
          // the same
          solver.reset(A);
          status *= verifyAnswer(*(solver.getLFactor()),
                                 rowsL_,
      ...
      private:
        ...
        ReSolve::MatrixHandler* createMatrixHandler()
        {
          if (memspace_ == "cpu") {
            ...
            std::cout << "ReSolve not built with support for memory space "
                      << memspace_ << "\n";
          }
          return nullptr;
        }
        ...
        std::vector<index_type> colsAL_ = {0, 0, 1, 3, 2, 4, 0, 2, 4};
        std::vector<real_type> valsAL_ = {
            1.0, 1.0, 4.0, 5.0, 2.0, 2.0, 2.0, 7.0, 8.0};
        ...
        std::vector<index_type> colsAU_ = {0, 4, 6, 1, 3, 5, 2, 4, 7, 3,
                                           5, 8, 4, 5, 6, 6, 7, 7, 8, 8};
        std::vector<real_type> valsAU_ = {2.0, 1.0, 3.0, 7.0, 5.0, 4.0, 0.1,
                                          3.0, 2.0, 3.0, 2.0, 8.0, 0.1, 1.0,
                                          6.0, 3.0, 3.0, 5.0, 1.0, 4.0};

        // Incomplete factor L of the test matrix (assumes zero_diagonal_ =
        // 0.1):
        //
        //     [                                                              ]
        //     [     0                                                        ]
        ...
        // Incomplete factor U of the test matrix (assumes zero_diagonal_ =
        // 0.1):
        //
        //     [     2      0      0      0      1      0      3      0      0]
        //     [            7      0      5      0      4      0      0      0]
        ...
        std::vector<index_type> colsU_ = {0, 4, 6, 1, 3, 5, 2, 4, 7, 3,
                                          5, 8, 4, 5, 6, 6, 7, 7, 8, 8};
        std::vector<real_type> valsU_ = {
            2.0, 1.0, 3.0,   7.0, 5.0, 4.0,  0.1,
            2.5, 2.0, 3.0,   2.0, 8.0, -0.4, -2.714285714285714,
            6.0, 3.0, -37.0, 5.0, 1.0, 4.0};
