...
namespace ReSolve
{
  namespace tests
  {
    ...
    {
    public:
      MatrixHandlerTests(ReSolve::MatrixHandler& handler)
        : handler_(handler)
      {
      ...
      bool verifyCsrMatrix(matrix::Csr* A, real_type val = 0.0)
      {
        ...
        if (n == m)
        {
          for (index_type i = 0; i < m; ++i)
          {
            if (i == m - 1)
            {
              if (rowptr_csr[i + 1] != rowptr_csr[i] + 1 || colidx_csr[rowptr_csr[i]] != n - 1 || val_csr[rowptr_csr[i]] != 2.0 * n + val)
              {
                return false;
              }
            }
            ...
            {
              if (rowptr_csr[i + 1] != rowptr_csr[i] + 3 || colidx_csr[rowptr_csr[i]] != 0 || val_csr[rowptr_csr[i]] != 2.0 + val || colidx_csr[rowptr_csr[i] + 1] != n / 2 || colidx_csr[rowptr_csr[i] + 2] != n / 2 + 1 || val_csr[rowptr_csr[i] + 1] != 2.0 * (n / 2) + 2 + val || val_csr[rowptr_csr[i] + 2] != 2.0 * (n / 2) + 3 + val)
              {
                return false;
              }
            }
            ...
            {
              if (rowptr_csr[i + 1] != rowptr_csr[i] + 2 || colidx_csr[rowptr_csr[i]] != i || colidx_csr[rowptr_csr[i] + 1] != i + 1)
              {
                return false;
              }
              if (i == 0)
              ...
              {
                if (val_csr[rowptr_csr[i]] != 2.0 * (i + 1) + val || val_csr[rowptr_csr[i] + 1] != 2.0 * (i + 1) + 1.0 + val)
                {
                  return false;
                }
              }
        ...
        else if (n > m)
        {
          ...
          for (index_type i = 0; i < m; ++i)
          {
            if (rowptr_csr[i + 1] != rowptr_csr[i] + 2 || colidx_csr[rowptr_csr[i]] != main_diag_ind++ || colidx_csr[rowptr_csr[i] + 1] != off_diag_ind++ || val_csr[rowptr_csr[i]] != main_val++ || val_csr[rowptr_csr[i] + 1] != off_val++)
            {
              return false;
            }
            if (i >= n - m - 1)
        ...
        else
        {
          ...
          for (index_type i = 0; i < m; ++i)
          {
            if (i < n && i < m - n)
            {
              if (rowptr_csr[i + 1] != rowptr_csr[i] + 1 || colidx_csr[rowptr_csr[i]] != i || val_csr[rowptr_csr[i]] != main_val)
              {
                return false;
              }
              main_val += 2.0;
            ...
            {
              if (rowptr_csr[i + 1] != rowptr_csr[i] + 2 || colidx_csr[rowptr_csr[i] + 1] != i || colidx_csr[rowptr_csr[i]] != i + n - m || val_csr[rowptr_csr[i] + 1] != main_val || val_csr[rowptr_csr[i]] != off_val)
              {
                return false;
              }
              main_val += 2.0;
            ...
            {
              if (rowptr_csr[i + 1] != rowptr_csr[i] + 1 || colidx_csr[rowptr_csr[i]] != i + n - m || val_csr[rowptr_csr[i]] != off_val)
              {
                return false;
              }
              off_val += 2.0;
