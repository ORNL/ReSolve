...
public:
  ...
  TestOutcome cooToCsr()
  {
    ...
    index_type n_answer = symmetric_expanded_csr_matrix_rows_.size() - 1;

    std::istringstream file2(symmetric_coo_matrix_file_);
    matrix::Coo* A = ReSolve::io::readMatrixFromFile(file2);
    ReSolve::matrix::Csr* A_csr = new matrix::Csr(n_answer, n_answer, 10);

    index_type nnz_coo = 10;
    if (A->getNnz() != nnz_coo) {
...
1 1 1.050000000000000e+01
2 2 1.500000000000000e-02
0 3 6.000000000000000e+00
3 1 2.505000000000000e+02
3 3 -2.800000000000000e+02
3 4 3.332000000000000e+01
4 4 1.200000000000000e+01
)";
...
%
 ...
 2  4  12.0 
  ...
  /// Matching expanded CSR matrix data as it is supposed to be read from the file
