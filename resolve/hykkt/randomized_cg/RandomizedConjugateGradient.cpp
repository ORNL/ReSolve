...
namespace ReSolve
{
  namespace hykkt
  {
     ...
     */
    void RandomizedConjugateGradient::addMatrixInfo(matrix::Csr* A, matrix::Csr* A_tr)
    {
      A_    = A;
      A_tr_ = A_tr;
    }
    ...
    {
      // skip diagonal preconditioning for now
      A_prec_ = A_;
      A_prec_tr_ = A_tr_;
      b_norm_ = sqrt(vector_handler_->dot(b_, b_, memspace_));
      vector_handler_->scal(1.0 / b_norm, b_prec) // times D_
