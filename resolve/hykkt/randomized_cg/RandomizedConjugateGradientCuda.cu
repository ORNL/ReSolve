...
namespace ReSolve
{
  ...
  namespace hykkt
  {
    namespace kernels
    {
      ...
      template <index_type K>
      __global__ __launch_bounds__(256, 2) void SpMMTallSkinnyKernelVector(
        const index_type* __restrict__ A_row_ptr,
        const index_type* __restrict__ A_col_idx,
        const real_type* __restrict__ A_val,
        const real_type* __restrict__ B,
      ...
      // W = W - B * L^-1
      template <index_type k>
      __global__ void updateW(real_type* W, const real_type* L, const real_type* B, index_type n)
      {
        index_type thread = blockIdx.x * blockDim.x + threadIdx.x;
        index_type stride = gridDim.x * blockDim.x;
    ...
    int RandomizedConjugateGradientCuda::SpMMTallSkinny(matrix::Csr* A, vector::Vector* X, vector::Vector* result)
    {
      ...
      {
      case 1:
        kernels::SpMMTallSkinnyKernelVector<1><<<num_blocks, block_size>>>(A->getRowData(memory::DEVICE),
        ...
        break;
      case 2:
        kernels::SpMMTallSkinnyKernelVector<2><<<num_blocks, block_size>>>(A->getRowData(memory::DEVICE),
                                                                A->getColData(memory::DEVICE),
                                                                A->getValues(memory::DEVICE),
                                                                X->getData(memory::DEVICE),
                                                                result->getData(memory::DEVICE),
        ...
        break;
      case 4:
        kernels::SpMMTallSkinnyKernelVector<4><<<num_blocks, block_size>>>(A->getRowData(memory::DEVICE),
                                                                A->getColData(memory::DEVICE),
                                                                A->getValues(memory::DEVICE),
                                                                X->getData(memory::DEVICE),
                                                                result->getData(memory::DEVICE),
        ...
        break;
      case 8:
        kernels::SpMMTallSkinnyKernelVector<8><<<num_blocks, block_size>>>(A->getRowData(memory::DEVICE),
                                                                A->getColData(memory::DEVICE),
                                                                A->getValues(memory::DEVICE),
                                                                X->getData(memory::DEVICE),
                                                                result->getData(memory::DEVICE),
        ...
        break;
      case 16:
        kernels::SpMMTallSkinnyKernelVector<16><<<num_blocks, block_size>>>(A->getRowData(memory::DEVICE),
                                                                A->getColData(memory::DEVICE),
                                                                A->getValues(memory::DEVICE),
                                                                X->getData(memory::DEVICE),
                                                                result->getData(memory::DEVICE),
