...
namespace ReSolve
{
  namespace cuda
  {
    namespace kernels
    {
      ...
      // ...
      __global__ void extractDiagonal(index_type        n,
                                             const index_type* a_row_ptr,
                                             const index_type* a_col_ind,
                                             const real_type*  a_val,
                                             index_type* d_row_ptr,
                                             index_type* d_col_ind,
                                             real_type*  d_val)
      {
        // Get row index from thread and block indices
        index_type row = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        if (row < n)
        {
          // Since D is a diagonal matrix, the row index is also the column index
          d_col_ind[row] = row;
                                             ...
                                             real_type*        d_val)
      {
        ...
        if (row < n)
        {
          ...
          for (index_type i = 0; i < row_end - row_start; i++)
          {
            if (a_col_ind[a_row_ptr[row] + i] == row)
            {
              d_val[row] = 1.0 / sqrt(a_val[a_row_ptr[row] + i]);
            }
          }
        }
      }
                                ...
                                real_type*  d_val)
    {
      // Define block size and number of blocks
      const int block_size = 256;
      int       num_blocks = (n + 1 + block_size - 1) / block_size;
      // Launch the kernel
      kernels::extractDiagonal<<<num_blocks, block_size>>>(n, a_row_ptr, a_col_ind, a_val, d_row_ptr, d_col_ind, d_val);
    }
                                ...
                                real_type*        d_val)
    {
      ...
      int       num_blocks = (n + block_size - 1) / block_size;
      // Launch the kernel
      kernels::extractInverseRootDiagonal<<<num_blocks, block_size>>>(n, a_row_ptr, a_col_ind, a_val, d_val);
    }
