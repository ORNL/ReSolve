...
// NOTE: probably not safe to use. there's a mistake of assuming n_ ==
//       number of columns all throughout this file, when n_ is actually the
//       number of rows (see Sparse.cpp). don't use Csc until this is fixed
...
namespace ReSolve
{
                              ...
                              memory::MemorySpace memspaceOut)
  {
    ...
    if (memspaceOut == memory::HOST) {
      ...
      if ((h_row_data_ == nullptr) != (h_col_data_ == nullptr)) {
        out::error()
            << "In Csc::updateData one of host row or column data is null!\n";
      }
    ...
    if (memspaceOut == memory::DEVICE) {
      ...
      if ((d_row_data_ == nullptr) != (d_col_data_ == nullptr)) {
        out::error()
            << "In Csc::updateData one of device row or column data is null!\n";
      }
                              ...
                              memory::MemorySpace memspaceOut)
  {
    ...
    this->nnz_ = new_nnz;
    int i =
        this->updateData(col_data, row_data, val_data, memspaceIn, memspaceOut);
    return i;
  }
    ...
    switch (memspaceOut) {
      case HOST:
        if ((d_data_updated_ == true) && (h_data_updated_ == false)) {
          if ((h_row_data_ == nullptr) != (h_col_data_ == nullptr)) {
            out::error()
                << "In Csc::copyData one of host row or column data is null!\n";
          }
          if ((h_col_data_ == nullptr) && (h_row_data_ == nullptr)) {
      ...
      case DEVICE:
        if ((d_data_updated_ == false) && (h_data_updated_ == true)) {
          if ((d_row_data_ == nullptr) != (d_col_data_ == nullptr)) {
            out::error() << "In Csc::copyData one of device row or column data "
                            "is null!\n";
          }
          if ((d_col_data_ == nullptr) && (d_row_data_ == nullptr)) {
            mem_.allocateArrayOnDevice(&d_col_data_, n_ + 1);
  ...
  void matrix::Csc::print(std::ostream& out)
  {
    ...
    for (index_type i = 0; i < m_; ++i) {
      for (index_type j = h_col_data_[i]; j < h_col_data_[i + 1]; ++j) {
        out << h_row_data_[j] << " " << i << " " << h_val_data_[j] << "\n";
      }
    }
  }
} // namespace ReSolve
