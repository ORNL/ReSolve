...
namespace ReSolve
{
  ...
  public:
    ...
    MatrixElementTriplet()
      : rowidx_(0), colidx_(0), value_(0.0)
    {
    }

    /// Constructor that initializes row and column indices and the element value.
    MatrixElementTriplet(index_type i, index_type j, real_type v)
      : rowidx_(i), colidx_(j), value_(v)
    {
    }
  ...
  namespace io
  {
    ...
    vector::Vector* createVectorFromFile(std::istream& file)
    {
      ...
      vector::Vector* vec = new vector::Vector(n, m);
    ...
    int writeVectorToFile(vector_type* vec_x, std::ostream& file_out)
    {
      ...
      {
        file_out << vec_x->getSize() << " " << "\n";
      }
      else
      {
        file_out << vec_x->getSize() << " " << vec_x->getNumVectors() << " " << "\n";        
      }
      for (int i = 0; i < vec_x->getSize() * vec_x->getNumVectors(); ++i)
      {
