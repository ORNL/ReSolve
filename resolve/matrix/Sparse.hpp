...
namespace ReSolve
{
  ...
  {
    /**
     * @brief This class implements basic sparse matrix interface. (Almost) all
     * sparse matrix formats store information about matrix rows and columns (as
     * integers) and data (as real numbers). This class is virtualand implements
     * only what is common for all basic formats. Note that regardless of how
     * row/column information is stored, all values need to be stored, so all
     * utilities needed for values are implemented in this class.
     *
    ...
    class Sparse
    {
      public:
        ...
        void
        setNnz(index_type nnz_new); // for resetting when removing duplicates
        ...
        int setMatrixData(index_type* row_data,
                          index_type* col_data,
                          real_type* val_data,
        ...
        virtual int updateValues(real_type* new_vals,
                                 memory::MemorySpace memspaceIn,
                                 memory::MemorySpace memspaceOut);

        // set new values just sets the pointer, use caution.
        virtual int setNewValues(real_type* new_vals,
                                 memory::MemorySpace memspace);
      ...
      protected:
        ...
        index_type nnz_expanded_{0}; ///< number of non-zeros in an expanded
                                     ///< matrix (for symmetric matrices)
