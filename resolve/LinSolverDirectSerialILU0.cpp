    ...
    for (index_type i = 0; i < n; ++i) {
      ...
      for (index_type j = j1; j < j2; ++j) {
        ...
        if (A->getColData(ReSolve::memory::HOST)[j] == i) {
          ...
          U_->getValues(ReSolve::memory::HOST)[uit] = A_->getValues(ReSolve::memory::HOST)[i];
        ...
        if (A->getColData(ReSolve::memory::HOST)[j] > i) {
          ...
          U_->getValues(ReSolve::memory::HOST) [uit] = A_->getValues(ReSolve::memory::HOST) [i]; 
          U_->getColData(ReSolve::memory::HOST)[uit] = A_->getColData(ReSolve::memory::HOST)[i]; ;
        ...
        if (A->getColData(ReSolve::memory::HOST)[j] < i) {
          // lower part
          L_->getValues(ReSolve::memory::HOST) [lit]  = A_->getValues(ReSolve::memory::HOST)[i]; 
          L_->getColData(ReSolve::memory::HOST)[lit] = A_->getColData(ReSolve::memory::HOST)[i]; 
